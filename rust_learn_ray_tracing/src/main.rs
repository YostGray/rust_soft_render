...
use rust_learn_ray_tracing::rt_lib::{scene::Scene, cam::Camera, vector3::Vector3, sphere::Sphere, material::{mat_lambertian::Lambertian, mat_metal::Metal, mat_dielectric::Dielectric}};
...
fn main(){
    ...
    let mat_right = Arc::new(Metal::new(Vector3::new(0.8, 0.8, 0.8),0.3));
    ...
    s.add_obj(Box::new(Sphere::new(&Vector3::new(-2.0, 0.0, -10.0), 1.0,mat_left.clone())));
    s.add_obj(Box::new(Sphere::new(&Vector3::new(-2.0, 0.0, -10.0), -0.98,mat_left.clone())));
    ...
    println!("\nTime cost in render() is: {:?}", duration);
    img.liner_to_gamma();

    match bmp::save_as_file(&img, "./test_out/test11-shell2.bmp",BitDepth::AllColors) {
        Err(msg) => {
            panic!("{}",msg);
        },
        _ => (),
